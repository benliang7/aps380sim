...
data = readtable("MichiganEndurance.xlsx");
cols = table2array(data);
Torque = [cols(:,1), cols(:,2)];
RPM = [cols(:,1), cols(:,3)];
...
battery.DCIR = 0.025;                                   % Cell internal resistance [Ohm]
battery.S = 115;                                        % Number of series cells [-]
battery.P = 5;                                          % Number of parallel cells [-]
battery.Ah = 2.735;                                       % Cell capacity [Ah]
...
% 4 AWG AC cable
rac = 2.5945e-3;                             % AC cable radius [m]
lac = 0.35;                                  % AC cable length [m]

powertrain.Rph = 1.7 * 10^-8 * lac / (pi*rac^2);        % Phase cable resistance [Ohm]
...
motor.Rw = 0.0167 + powertrain.Rph;                     % Motor Winding resistance [ohm]
